...
export const Todo = ({ todo }) => {
  ...
  const deleteTodo = () => {
    todoService.delete(todo.id).then((response) => {
      if (response.ok) {
        dispatch({
          type: TODO_ACTIONS.TODO_DELETED,
          playload: { todoId: todo.id, listId: list.id },
        });
      }
    });
  };
  ...
  return (
    <tr style={todo.isCompleted ? decorationDone : {}}>
      <td>{todo.label}</td>
      <td>
        <input type="" defaultChecked={todo.isCompleted}>
          {" "}
        </input>
      </td>
      <td>
        {" "}
        <button onClick={deleteTodo}>Eleminar</button>{" "}
      </td>
      <td>
        {" "}
