...
export const List = ({ item }) => {
    ...
    const [todoName, setTodoName] = useContext();
    
    const createTodo = (event) =>{
        event.preventDefault();
        const body = {id: uuidv4(), label: todoName, isCompleted: false, listId: list.id};
        service.create(body)
        .then(response => response.json())
        .then(response => {
            dispatch({ type: TODO_ACTIONS.TODO_CREATED, playload: response});
    ...
    const deleteList = () => {
        ...
        .then(response =>{
            if(response.ok) {
                dispatch({type: LIST_ACTIONS.LIST_DELETED, playload: item.id})
            }
        })
    }
